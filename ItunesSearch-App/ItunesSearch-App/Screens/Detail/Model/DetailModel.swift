...
final class DetailModel{
    ...
    func fetchIdResults(for idList: [Int], with query: CachingQuery) {
        ...
        if internet.isOnline() {
            network.fetchById(with: idList, dtoType: dtoDetail.self) { [weak self] response in
                ...
                case .success(let data):
                    ...
                    query.isCacheMiss ? self.delegate?.didFetchCacheMissData(with: idList, for: query.cellIndexPath) : self.delegate?.didFetchDetailData()
                case .failure(_):
